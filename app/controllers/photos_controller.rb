class PhotosController < ApplicationController
  ...
  def index
    ...
    if !params[:photo_album_id].nil?
      @photos = Photo.where(photo_album_id: params[:photo_album_id]).approved.order("updated_at DESC, position ASC").limit(params[:limit]).offset(offset)
    elsif !params[:program_id].nil?
      @photos = Photo.where(program_id: params[:program_id]).approved.order("updated_at DESC, position ASC").limit(params[:limit]).offset(offset)
    end
